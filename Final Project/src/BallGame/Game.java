...
public class Game {
    ...
    Label scale = new Label("Scale: 1 pixel = 1 cm");
    ...
    public void runGame() {
        ...
        TopPane.getChildren().setAll(MenuBox,time_AttempsBox,scale);
    ...
    public void createTrajectoryLine(Scene scene, Line trajectoryLine) {
        trajectoryLine.setStroke(Color.GOLD);
            ...
            for (int i = 1; i < level.walls.size(); i++) {
                ...
                if(PhysicsEngine.collides(level.ball, level.walls.get(i))){
                    if(level.ball.kE()>=level.walls.get(i).getStrength()){
                        ...
                        level.ball.setVelocityX(level.ball.getVelocityX()*(1-level.walls.get(i).getStrength()/level.ball.kE()));
                        level.ball.setVelocityY(level.ball.getVelocityY()*(1-level.walls.get(i).getStrength()/level.ball.kE()));
                        level.walls.get(i).breaks();
                    }
