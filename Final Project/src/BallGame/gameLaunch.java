...
public class gameLaunch {
    ...
    public Scene endScene(BallCollisionGame Application,Stage stage){
        ...
        HBox buttonBox = new HBox(30,prevButton,retryButton,nextButton);
        ...
        if(Objects.isNull(golfgame.gameWon)){
            return    levelScene(Application,stage,new File(Application.levelsProperties.get(level.index)));                
        }else if(golfgame.gameWon){
            endLabel.setText("Congratulations");
            if(level.index>=Application.levelsProperties.size()-1){
                nextButton.setText("No More Levels");
