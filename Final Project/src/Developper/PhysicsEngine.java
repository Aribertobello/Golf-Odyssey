    ...
    public static boolean collides(Body body1, Body body2){
        ...
        if(body2 instanceof Wall){
            ...
            if(!((body1.speed()==0)&&(body2.speed()==0))){
                if(!(body2.getBounds()[1]==0)){
                    x = (-Math.sqrt(a*a*(r*r-h*h)+2*b*(a*h-k)+2*a*h*k-b*b-k*k+r*r)+a*(b+k)+h)/(a*a+1);
                    y = (a*(-Math.sqrt(a*a*(r*r-h*h)+2*b*(a*h-k)+2*a*h*k-b*b-k*k+r*r)+a*(k)+h)-b)/(a*a+1);
                    if((Math.max(((Wall) body2).startX,((Wall) body2).endX)<x||Math.min(((Wall) body2).startX,((Wall) body2).endX)>x)||(Math.max(((Wall) body2).startY,((Wall) body2).endY)<y||Math.min(((Wall) body2).startY,((Wall) body2).endY)>y)){
                        x = Double.NaN;
                    }
                    if(!(Double.isNaN(x))){
                        return true;
                    ...
                    y = k-Math.sqrt(-b*b+2*b*h-h*h+r*r);
                    if(Math.max(((Wall) body2).startY,((Wall) body2).endY)+r<y||Math.min(((Wall) body2).startY,((Wall) body2).endY)>y-r){
                        y = Double.NaN;
                    }
                    if(!(Double.isNaN(y))){
                        return true;
